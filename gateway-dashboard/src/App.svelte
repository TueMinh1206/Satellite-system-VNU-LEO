<script lang="ts">
  ...
  type WsEvent = {
    type: string;          // "TELEMETRY", "HANDOVER_START", "HANDOVER_DONE"
    state: string;         // "connected", "handover", ...
    satellite_id: string;
    gateway: string;
    elevation: number;
    azimuth: number;
  ...
  let selectedGateway: string | null = null;   // gateway hiện tại (engine chọn)

  let pollInterval: number;
  let phasedInterval: number;

  // ── Fetch từ Node.js (port 3001) ────────────────────────────────────
  async function fetchData() {
    try {
      const [satRes, obsRes, histRes] = await Promise.all([
        ...
        fetch("http://localhost:3001/api/handover-history"),
      ]);
      if (!satRes.ok) return;
      ...
      // bestSatellite & gatewaysView từ Node.js
      bestSatellite = satData.bestSatellite;
      gatewaysView = satData.gatewaysView ?? [];

      // Lịch sử handover: Node.js trả về { from_sat, to_sat, from_gw, to_gw, quality, cn, timestamp, type }
      handoverHistory = histData.map((ev: any) => ({
        satellite: ev.to_sat,
        gateway: ev.to_gw,
        quality: ev.quality,
        cn: ev.cn,
      ...
      gateways = obsData.gateways.map((gw: any, i: number) => ({
        ...
        traffic: 0,   // có thể thay bằng số liệu thực sau
        ping: 0,
      }));

      // Satellites visible (từ satData.router hoặc satData.gateways)
      // Tạo danh sách vệ tinh từ gateways (mỗi gateway có danh sách visible)
      const seen = new Set<string>();
      const allSats: Satellite[] = [];
      if (satData.gateways) {
        satData.gateways.forEach((gw: any, idx: number) => {
          if (gw.visible && Array.isArray(gw.visible)) {
            gw.visible.forEach((s: any) => {
              const name = s.tleArr[0];
              if (!seen.has(name)) {
                allSats.push({
                  id: name,
                  visibleGateway: `GW-${idx}`,  // hoặc gw.name
      ...
      // Tạo router và session (chỉ để hiển thị, không ảnh hưởng handover)
      if (bestSatellite) {
        routers = [{
          mac: "00:1A:2B:VNU",
          type: "Fixed",
          ...
          signalQuality: bestSatellite.quality > 70 ? "Good" : bestSatellite.quality > 40 ? "Medium" : "Poor",
          antennaLock: true,
  ...
  // ── GET phased-array từ api.py (Python) ────────────────────────────
  async function fetchPhasedArray() {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/api/phased-array?el=${el}&az=${az}&rain=20`
      );
  ...
  function connectWS() {
    try {
      ...
      ws.onmessage = (e) => {
        ...
        wsEvent = data;
        if (data.state) {
          sessionState = (data.state.charAt(0).toUpperCase() + data.state.slice(1)) as SessionStatus;
        }
        if (data.handover_count !== undefined) {
          handoverCount = data.handover_count;
        }
        if (data.gateway) {
          selectedGateway = data.gateway;
          // Cập nhật lại session/router với gateway mới
          if (sessions.length > 0) sessions[0].gatewayId = selectedGateway;
          if (routers.length > 0) routers[0].assignedGW = selectedGateway;
          sessions = [...sessions];
          routers = [...routers];
        }
        if (data.type === "HANDOVER_DONE") {
          // Thêm ngay lập tức vào lịch sử (để hiển thị mà không cần đợi fetch lại)
          handoverHistory = [
            {
          ...
          // Cập nhật biểu đồ anten
          fetchPhasedArray();
        }
        if (data.type === "HANDOVER_START") {
          fetchPhasedArray();
        }
      };
  ...
  onMount(() => {
    fetchData();
    connectWS();
    pollInterval = window.setInterval(fetchData, 15000);    // 15 giây thay vì 5
    phasedInterval = window.setInterval(fetchPhasedArray, 10000);
  });
  onDestroy(() => {
...
<main class="dashboard">
  ...
  {#if selectedGatewayId || selectedSatelliteId}
    <div class="filter-bar">
      <span>Đang lọc: <strong>{selectedGatewayId ?? "Tất cả"}</strong>
  ...
  <div class="grid-container">
    ...
    <div class="left-col">
      <section class="panel">
        ...
        <!-- Controls đã được khóa cứng, không hiển thị nữa -->
        <PolarChart data={phasedData} satellite={bestSatellite} />
        {#if phasedData?.budget}
          <div class="budget-strip">
            <div class="bitem"><span class="blabel">C/N₀</span><span class="bval accent">{phasedData.budget.CN0_dBHz} dBHz</span></div>
            <div class="bitem"><span class="blabel">C/N</span><span class="bval" style="color:{qualityColor}">{phasedData.budget.CN_db} dB</span></div>
            <div class="bitem"><span class="blabel">FSPL</span><span class="bval warn">{phasedData.budget.fspl_db} dB</span></div>
            <div class="bitem"><span class="blabel">Margin</span><span class="bval" class:good={phasedData.budget.link_ok} class:bad={!phasedData.budget.link_ok}>{phasedData.budget.link_margin_db > 0 ? "+" : ""}{phasedData.budget.link_margin_db} dB</span></div>
            <div class="bitem"><span class="blabel">Link</span><span class="bval" class:good={phasedData.budget.link_ok} class:bad={!phasedData.budget.link_ok}>{phasedData.budget.link_ok ? "✓ OK" : "✗ Fail"}</span></div>
          </div>
        {/if}
      </section>
      ...
      <section class="panel mt-4">
        ...
        <div class="satellite-grid">
          {#each displaySatellites as sat}
            <div class="sat-chip interactive" class:active-sat={selectedSatelliteId === sat.id} class:best-sat={sat.id === bestSatellite?.id} on:click={() => toggleSatellite(sat.id)}>
              <div class="sat-title">{sat.id}</div>
              <div class="sat-info"><span>El {sat.elevation}°</span><span>Az {sat.azimuth}°</span></div>
              {#if sat.id === bestSatellite?.id}<div class="tracking-badge">● Tracking</div>{/if}
            </div>
          {/each}
          {#if displaySatellites.length === 0}<p class="text-muted">Không có vệ tinh nào.</p>{/if}
        </div>
      </section>
    </div>
    ...
    <div class="right-col">
      <section class="panel">
        <h2>🌐 Gateways {#if selectedGateway}<span class="selected-gw-badge">Engine → {selectedGateway}</span>{/if}</h2>
        <div class="card-list">
          {#each gateways as gw}
            <div class="card interactive" class:border-green={gw.status === "Alive"} class:border-red={gw.status === "Dead"} class:active-gw={selectedGatewayId === gw.id} class:engine-selected={gw.name === selectedGateway} on:click={() => toggleGateway(gw.id)}>
              <div class="card-header"><h3>{gw.name}{#if gw.name === selectedGateway}<span class="engine-tag">▲ Engine</span>{/if}</h3><span class="badge" class:bg-green={gw.status==="Alive"} class:bg-red={gw.status==="Dead"}>{gw.status}</span></div>
              <div class="card-body">
                {#each gatewaysView.filter(g => g.name === gw.name) as gv}
                  <p>El vệ tinh: <strong>{gv.elevation?.toFixed(1) ?? "N/A"}°</strong></p>
                  <p>C/N: <strong class:good={gv.cn > 10} class:warn={gv.cn <= 10}>{gv.cn?.toFixed(1) ?? "N/A"} dB</strong></p>
                {/each}
      ...
      <section class="panel mt-4">
        ...
        <div class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>Session</th><th>Router</th><th>Satellite</th><th>Gateway</th><th>Status</th><th>Latency/Loss</th></tr></thead>
            <tbody>
              {#each displaySessions as s}
                <tr><td>{s.id}</td><td><code>{s.routerMac.slice(-5)}</code></td><td><span class="highlight">{s.satelliteId}</span></td><td>{s.gatewayId}</td><td><span class="status-dot {s.status.toLowerCase()}"></span>{s.status}</td><td>{s.latency}ms / {s.packetLoss}%</td></tr>
              {/each}
              {#if displaySessions.length === 0}<tr><td colspan="6" class="text-muted">Chưa có session.</td></tr>{/if}
            </tbody>
          </table>
        </div>
      </section>
          ...
          <table class="data-table">
            ...
            <tbody>
              {#each displayRouters as r}
                <tr><td><code>{r.mac}</code></td><td><span class="badge-outline">{r.type}</span></td><td class:text-warning={r.cnRatio < 15} class:text-good={r.cnRatio >= 15}>{r.cnRatio?.toFixed(1)} dB</td><td>{r.elevation}°</td><td style="color:{r.signalQuality === 'Good' ? '#22c55e' : r.signalQuality === 'Medium' ? '#f59e0b' : '#ef4444'}">{r.signalQuality}</td><td>{r.antennaLock ? "✅" : "❌"}</td></tr>
              {/each}
            </tbody>
          </table>
        </div>
      ...
      <div class="system-logs mt-4">
        <div class="logs-header"><h3>🔄 Lịch sử Handover</h3>{#if wsEvent}<span class="ws-live {wsEvent.type === 'HANDOVER_START' ? 'ws-warn' : wsEvent.type === 'HANDOVER_DONE' ? 'ws-good' : 'ws-info'}">⚡ {wsEvent.type}</span>{/if}</div>
        <ul>
          {#each handoverHistory as event}
            <li><span class="log-time">[{new Date(event.timestamp * 1000).toLocaleTimeString()}]</span><strong>{event.satellite}</strong> ➔ <span class="log-gw">{event.gateway}</span><span class="log-meta">Q:{event.quality}% C/N:{event.cn}dB</span></li>
          {/each}
          {#if handoverHistory.length === 0}<li class="text-muted">Chưa có bản ghi.</li>{/if}
        </ul>
      </div>
    </div>
  </div>
...
<style>
  :global(body) { margin:0; font-family:'Inter',system-ui,sans-serif; background:#07111f; color:#e2e8f0; }
  .dashboard { min-height:100vh; padding:20px 24px; box-sizing:border-box; }
  header { display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; gap:20px; padding-bottom:16px; border-bottom:1px solid #1e293b; }
  ...
  .stat-box { background:#0f1e33; border:1px solid #1e3a5f; border-radius:10px; padding:10px 14px; min-width:120px; }
  ...
  .filter-bar { background:#1e3a8a; border-left:4px solid #38bdf8; padding:10px 16px; border-radius:8px; margin-bottom:16px; display:flex; justify-content:space-between; align-items:center; font-size:14px; }
  .btn-clear { background:rgba(255,255,255,0.1); border:none; color:white; padding:5px 12px; border-radius:4px; cursor:pointer; }
  .btn-clear:hover { background:#ef4444; }
  .grid-container { display:grid; grid-template-columns:380px 1fr; gap:16px; }
  @media (max-width:1100px) { .grid-container { grid-template-columns:1fr; } }
  .panel { background:#0b1728; border:1px solid #1e293b; border-radius:12px; padding:16px; }
  .panel h2 { margin:0 0 14px; font-size:15px; font-weight:600; color:#94a3b8; border-bottom:1px solid #1e293b; padding-bottom:10px; display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
  .mt-4 { margin-top:16px; }
  .satellite-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; max-height:280px; overflow-y:auto; }
  .sat-chip { background:#111f35; padding:10px; border-radius:8px; border:1px solid transparent; cursor:pointer; transition:0.15s; }
  ...
  .card { background:#111f35; border-radius:10px; padding:12px; border:1px solid transparent; border-left:4px solid #475569; cursor:pointer; transition:0.15s; }
  ...
  .selected-gw-badge { font-size:11px; color:#f59e0b; background:rgba(245,158,11,0.1); padding:2px 8px; border-radius:4px; font-weight:600; }
  ...
  .budget-strip { display:flex; gap:4px; margin-top:10px; background:#060d1a; border-radius:8px; padding:10px; flex-wrap:wrap; }
