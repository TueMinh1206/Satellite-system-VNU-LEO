<script lang="ts">
  ...
  async function fetchData() {
    try {
      ...
      if (!satRes.ok || !obsRes.ok || !histRes.ok)
  ...
  $: displayRouters = selectedSatelliteId
    ...
    : selectedGatewayId
      ...
      : routers;
  $: totalTraffic = gateways.reduce(
    (sum, gw) => sum + (gw.status === "Alive" ? gw.traffic : 0),
    0,
  );
...
<main class="dashboard">
  <header>
    ...
    <div class="global-stats">
      <div class="stat-box">
        <span class="label">Tổng lưu lượng</span><span class="value value-neon"
          >{totalTraffic} Mbps</span
        >
      </div>
      <div class="stat-box">
        <span class="label">Gateway Active</span><span class="value"
          >{activeGateways}/{gateways.length}</span
        >
      </div>
      <div class="stat-box">
        <span class="label">Active Sessions</span><span class="value"
          >{activeSessions}/{sessions.length}</span
        >
      </div>
    </div>
  </header>
    ...
    <div class="filter-bar">
      <span
        ...
        {#if selectedSatelliteId}
          ➔ <strong>{selectedSatelliteId}</strong>
        {/if}
      </span>
    ...
    <div class="left-col">
      <section class="panel">
        ...
        <div class="card-list">
          {#each gateways as gw}
            <div
              class="card interactive {selectedGatewayId === gw.id
                ? 'active-gw'
                : ''}"
            ...
            >
              <div class="card-header">
                ...
                <span
                  class:badge={true}
                  class:bg-green={gw.status === "Alive"}
                  class:bg-red={gw.status === "Dead"}>{gw.status}</span
                >
              </div>
              <div class="card-body">
                ...
                <p>
                  Ping: <strong
                    >{gw.status === "Alive" ? gw.ping + " ms" : "N/A"}</strong
                  >
                </p>
              </div>
            </div>
      ...
      <section class="panel mt-4">
        ...
        <div class="satellite-grid">
          {#each displaySatellites as sat}
            <div
              class="sat-chip interactive {selectedSatelliteId === sat.id
                ? 'active-sat'
                : ''}"
      ...
      <section class="panel">
        ...
        <div class="table-wrapper">
          <table class="data-table">
            <thead
              ><tr
                ><th>Session</th><th>Router</th><th>Satellite</th><th
                  >Gateway</th
                ><th>Status</th><th>Lat/Loss</th></tr
              ></thead
            >
            <tbody>
              {#each displaySessions as session}
                <tr>
                  ...
                  <td
                    ><span class="status-dot {session.status.toLowerCase()}"
                    ></span>{session.status}</td
                  >
      ...
      <section class="panel mt-4">
        ...
        <div class="table-wrapper">
          <table class="data-table">
            <thead
              ><tr
                ><th>MAC</th><th>Type</th><th>C/N</th><th>Elevation</th><th
                  >Signal</th
                ><th>Lock</th></tr
              ></thead
            >
            <tbody>
              {#each displayRouters as router}
                <tr>
                  ...
                  <td
                    class:text-warning={router.cnRatio < 15}
                    class:text-good={router.cnRatio >= 15}
                    >{router.cnRatio} dB</td
                  >
